...
export const ShowActiveFiltersControl = ({ getFilters }) => {
    ...
    const getDisplayFilters = () => {
        ...
        if (radiusFilter) {
            ...
            const radiusFilterToDisplay = `Center: (Lat: ${round(coordinates[1])}), (Lng: ${round(coordinates[0])}), Radius: ${radius} km`;
            filtersToDisplay.push(radiusFilterToDisplay);
        }
        return filtersToDisplay.length > 0 ? filtersToDisplay : ["No Filters Active"];
    }
